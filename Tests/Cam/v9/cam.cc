...
void amem::exe_command() {
  ...
  while(true) {
	...
	// And we should add count responders.
      ...
      case COMP: // Compare with broadcast comparand and mask
	...
	for(int i = 0 ; i<CAM_ARRAY_SIZE; i++) {
	  ...
	  if(DEBUG>1) 
	    cout << am_array[i] << rdata << " was read " << endl << endl;
	}
	readdata = rdata;
	break;
	...
	/////////////////////////////////////////////////////////
      ...
      case CTAG: // Clear all Tags and Degree of Match counters
	...
	/////////////////////////////////////////////////////////
      ...
      case ITAG: // Invert all Tags
	if(DEBUG) cout << "Invert All Tags" << endl;
	...
	/////////////////////////////////////////////////////////
      ...
      case SEL1ST: // Select the First Tag (pick one)
	...
	/////////////////////////////////////////////////////////
      ...
      case HAMM: // Calculate Hamming Distance in DoM counters
	// does not use or change Tags 
	if(DEBUG) cout << "Calculate Hammng Distance" << endl;

	// Clear counters
	for(int i = 0; i<CAM_ARRAY_SIZE; i++) am_array[i].dom = 0; 
	...
	/////////////////////////////////////////////////////////
      ...
      case READNB: // Read Next Best Match
	// Read amword from word with best DoM and reset its tag
	// Assumes tags are already set
	// Do we want to output the DoM value itself?
	// This could/should replace wta since that is just first match
	if(DEBUG) cout << "Read Next Best Match" << endl;

	{  	
	  int best = 0;
	  int add = 0;
	  // find tagged word with best DoM
	  for(int i = 0; i<CAM_ARRAY_SIZE; i++) {
	    if ((am_array[i].tag == '1') && (am_array[i].dom > best)) {
	      best = am_array[i].dom;
	      add = i;    
	    }
	  }
	  // Read that word and reset its tag
	  am_array[add].tag = '0';
	  rdata = am_array[add].word;
	  dom = am_array[add].dom;
