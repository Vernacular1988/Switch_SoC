...
#define AMSZ 4                 // address bits

// commands for the controller, note: ERR is in position zero for null command
enum command_type {ERR, COMPARE, READ, WRITE};

// need to add more commands: count, best / winner take all, 
// tagged read, and write I/O signals
//

//for writing out command values we overload the << operator 
inline ostream& operator << ( ostream& os, const command_type& t )
{
  char *command_string[] = {"ERR", "COMPARE", "READ", "WRITE"};
  
  os << command_string[t] << endl;
  return os;
}
...
SC_MODULE(amem) {
  ...
  sc_in_clk clock;
  cam_word am_array[CAM_ARRAY_SIZE]; // array of cam words
  void exe_command();
