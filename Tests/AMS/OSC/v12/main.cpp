...
int sc_main(int argc, char* argv[])
{
  ...
  sca_tdf::sca_signal<double> osc[NN];
  sca_tdf::sca_signal<double> lpf[NN];



  double phc_gain = 0.05; //1.0/NN;//100	// phase comparitor gain
  double lp_fc = 25000; //500.0;	//10	// low pass filter cut off freq
  double vco_freq = 1000.0; 	// natural frequency of vco
  double kvco = 1000; //10.0;//1000 10		// control sensitivity for vco
  ...
  int ninety = (int) floor(0.25/(vco_freq*1e-7)); // # samples for 90 degree phase delay at vco_freq and sample rate
  ...
  // Now we set the weights
   for(int i=0; i<NN; i++) {
     for(int j=0; j<NN; j++) {
       ...
       con[i][j]->out(sw[i][j]);
       con[i][j]->set_timestep(0.1, SC_US);
     }
   }
  /********* tracing signals *************************/
